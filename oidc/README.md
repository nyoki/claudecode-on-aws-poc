# Claude Code on Amazon Bedrock — IAM OIDC Federation 方式

Cognito ユーザ認証 + IAM OIDC Federation を経由して Claude Code を Amazon Bedrock 上で利用するためのツール。STS `AssumeRoleWithWebIdentity` で一時クレデンシャルを取得する。
...
ユーザ (Claude Code)
  ...
  │
  ├─ STS AssumeRoleWithWebIdentity で AWS 一時クレデンシャルに変換
  │   └─ セッション名に ID Token の email を使用（CloudTrail でユーザ特定可能）
  │
  └─ Bedrock API を呼び出し
```
...
| 4 | IAM OIDC Provider | Cognito User Pool を OIDC IdP として IAM に登録 |
| 5 | IAM ロール | Bedrock の InvokeModel 権限（OIDC Provider を信頼、MaxSessionDuration 12 時間） |
...
- Keychain サービス名は `com.claudecode.bedrock-login-oidc`
