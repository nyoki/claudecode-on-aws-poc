...
両方式は Keychain サービス名が異なるため、同一マシンで併用できる。
...
```json
{
  "env": {
    ...
    "ANTHROPIC_DEFAULT_OPUS_MODEL": "jp.anthropic.claude-sonnet-4-6",
...
> **モデル設定について:** Opus の推論プロファイルは JP リージョンに存在しないため、`ANTHROPIC_DEFAULT_OPUS_MODEL` に Sonnet を指定している。これにより Claude Code が Opus を要求した場合でも JP リージョンの Sonnet 4.6 が使われる。

グローバル設定（`~/.claude/settings.json`）には Bedrock 設定を入れないこと。ワークスペースごとにアカウントや利用する API を切り替えたい場合は、プロジェクト単位で設定するのがおすすめ。
